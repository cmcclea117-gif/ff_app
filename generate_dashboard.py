...
def parse_projections_csv(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Extract position (remove rank number like "QB1" -> "QB")
            ...
            try:
                # ECR uses AVG as the consensus rank, not projection points
                # We'll use the overall RK as the ECR for now
                # The actual projection calculation will happen in JavaScript
                ...
                # Store ECR rank - JavaScript will convert to points
                projections.append({
                    'p': player,
                    'pos': pos,
