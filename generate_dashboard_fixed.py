...
function renderReliabilityTable() {{
  let data = Object.entries(FP_ACCURACY)
    .filter(([name, stats]) => stats.games >= 3)  // ✅ Keep your filter
    .map(([name, stats]) => {{
      // ✅ Keep your avgScore calculation
    ...
    // ❌ REMOVE: .sort((a, b) => b.correlation - a.correlation);
  
  // ✅ ADD: Let applySorting handle it
